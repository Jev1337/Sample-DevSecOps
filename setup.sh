...
deploy_siem_dashboards() {
    ...
    # Deploy the updated Grafana dashboards ConfigMap with SIEM dashboards
    log "Updating Grafana dashboards ConfigMap with SIEM dashboards..." "$YELLOW"
    microk8s kubectl apply -f k8s/grafana-dashboards-configmap.yaml
    ...
    log "Restarting Grafana to reload dashboards..." "$YELLOW"
    microk8s kubectl rollout restart deployment grafana -n monitoring
    
    # Wait for Grafana to be ready
    log "Waiting for Grafana to be ready..." "$YELLOW"
    microk8s kubectl wait --for=condition=available --timeout=120s deployment/grafana -n monitoring
    
    log "✅ SIEM dashboards deployed and will be auto-loaded by Grafana sidecar." "$GREEN"
    log "📋 Available SIEM dashboards:" "$CYAN"
    log "   • SIEM Overview - Security event monitoring and statistics" "$CYAN"
    log "   • Authentication Monitoring - Failed/successful login tracking" "$CYAN"
    log "   • System Audit & Security - Audit logs and system activity" "$CYAN"
